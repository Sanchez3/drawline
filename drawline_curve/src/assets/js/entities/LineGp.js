...
LineGp.prototype.drawLine = function (lines) {
    ...
    if (this.lineArray.length > 3) {
        ...
        for (var a = 0; a < this.lineArray.length - 2; a++) {
            t = this.lineArray[0];
            e = this.lineArray[1];
            n = .5 * (t.x + e.x);
            i = .5 * (t.y + e.y - this.wh);
            this.line.quadraticCurveTo(t.x, t.y- this.wh / 2, n, i);
        }
        t = this.lineArray[a];
        e = this.lineArray[a + 1];
        this.line.quadraticCurveTo(t.x, t.y - this.wh / 2, e.x, e.y - this.wh / 2);
        this.line.endFill();
